...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><video id="video"></video><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">删除</span></div>';
	var defaults = {
	...
	var sendBlob = function(url,xhr,file,formdata){
	    ...
	    console.log("====before====",fd.get("name"));
		xhr.send(fd);
	}
	...
	this.each(function(){
	  ...
	  fileObj = {
		  ...
		  uploadFileList : $("."+boxarea_class).find('.uploadify-queue'),
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					$("."+boxarea_class).html($html);
                                        $("."+boxarea_class).attr("val",file.name)
					upload.viewAsvideo("video");
					//判断是否显示已上传文件大小
		  ...
		  onProgress: function(file, loaded, total) {
				var eleProgress = $("."+boxarea_class).find('#fileupload_'+instanceNumber+'_'+file.index+' .uploadify-progress');
		  ...
		  funGetProgressWidth: function(index){
		  	var eleProgressBar = $("."+boxarea_class).find('#fileupload_'+instanceNumber+'_'+index+' .uploadify-progress-bar');
		  ...
		  funDealFiles: function(files) {
			...
			var fileCount = $("."+boxarea_class).find('.uploadify-queue-item').length;
			  for(var i=0,len=files.length;i<len;i++){
		  ...
		  funDeleteFile: function(index) {
			  ...
			  for (var i = 0,len=this.fileFilter.length; i<len; i++) {
					  ...
					  if (file.index == index) {
						  ...
						  $("."+boxarea_class).find('#fileupload_'+instanceNumber+'_'+index).fadeOut()
		  ...
		  funUploadFile: function(file) {
			  var xhr = false;
			  var originalFile = file;//保存原始未切割的文件
			  var thisfile = $("."+boxarea_class).find('#fileupload_'+instanceNumber+'_'+file.index);
					  ...
					  if (xhr.readyState == 4) {
						  ...
						  if (xhr.status == 200) {
								...
								if(fileObj.uploadOver){
								  ...
								  //在指定的间隔时间后删掉进度条
								  setTimeout(function(){
									$(".result").find('#fileupload_'+instanceNumber+'_'+originalFile.index).fadeOut();
...
console.log("0-0-0-0--",option.formData)
